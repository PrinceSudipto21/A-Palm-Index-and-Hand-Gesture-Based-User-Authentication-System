...
def security():
    # if required database not exist in the database
    ...
    if flag == 0:
        ...
        addAdmins()
        flag = 2
    # if required database is already exist in the database
    elif flag == 1:
...
def wait1():
    ...
    if k == 0:
        cv2.putText(img, 'This system already exist. So, let\'s check your palm.',
        ...
        cv2.putText(img, 'Otherwise system will not be unlocked.', (5, 70),
...
def wait():
    ...
    if k == 0:
        cv2.putText(img, 'This system is new. So, let''s read your palm.',
                    (10, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 0), 2, cv2.LINE_AA)
        if time.time() - initial >= 5:
            k = 1
            initial = time.time()
...
def connecting(j):
    global conn, mycursor
    if j == 0:
        conn = msql.connect(host="localhost", port="3306", user="root",
                            password="123456789")
    ...
    if results.multi_hand_landmarks and temp == 1:
        ...
        if s == 0 or securityEnter == 1:
            ...
            if flag == 2 and (time.time()-initial <= 5):
                cv2.putText(img, 'ADMIN ADDED', (20, 300), cv2.FONT_HERSHEY_SIMPLEX, 2, (255, 0, 0), 2, cv2.LINE_AA)
            elif a == 21 and (time.time()-initial <= 5):
                cv2.putText(img, 'UNLOCKED', (20, 300), cv2.FONT_HERSHEY_SIMPLEX, 3, (255, 0, 0), 2, cv2.LINE_AA)
            elif time.time()-initial <= 5:
                cv2.putText(img, 'SECURITY FEATURES DIDN\'T MATCHED', (10, 300), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
            else:
                securityEnter = 2
